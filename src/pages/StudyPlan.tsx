...
interface AIContext { exam_date: string | null; subjects: string[]; incomplete_tasks: string[]; review_content: string | null; }
...
type TabType = 'plan' | 'ai' | 'check' | 'settings';
const STUDY_PHASES = [
...
export default function StudyPlan() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const context: AIContext = { exam_date: prefExamDate || null, subjects: selectedAISubjects,
        incomplete_tasks: tasks.filter(t => t.status === 'pending').map(t => t.content), review_content: aiUserInput || null };
      const result = await invoke<TaskSuggestion[]>('generate_ai_plan', { sessionToken, context });
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 flex gap-2">
        {[{ key: 'plan', label: '📋 任务' }, { key: 'ai', label: '🤖 AI生成' }, { key: 'check', label: '✅ 检查' }, { key: 'settings', label: '⚙️ 设置' }].map((tab) => (
            ...
            {activeTab === 'ai' && (
              <div className="bg-slate-800/30 rounded-xl border border-white/5 p-4">
                ...
                {!aiConfig?.is_configured ? (
                  ...
                  <div className="space-y-4">
                    <div className="p-2 bg-emerald-500/10 rounded-lg text-emerald-400 text-xs">✓ {aiConfig.model_name.split('/').pop()}</div>
                    <div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6 m-4 border border-white/10">
            ...
            <div className="space-y-4">
              ...
              <div><label className="block text-sm text-slate-400 mb-2">模型</label><select value={modelName} onChange={(e) => setModelName(e.target.value)} className="w-full px-3 py-2 bg-slate-700/50 border border-white/10 rounded-lg text-white"><option value="Qwen/Qwen2.5-7B-Instruct">Qwen2.5-7B (免费)</option><option value="Qwen/Qwen2.5-14B-Instruct">Qwen2.5-14B</option><option value="deepseek-ai/DeepSeek-V2.5">DeepSeek-V2.5</option></select></div>
            </div>
            <div className="mt-6 flex justify-end gap-3">
